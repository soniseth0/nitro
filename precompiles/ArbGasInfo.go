...
// used by the multi-dimensional multi-constraint pricing model.
//
// Each constraint includes its resource weights, adjustment window, target throughput,
// and current backlog. Only resources with nonzero weights are returned.
func (con ArbGasInfo) GetMultiGasPricingConstraints(
	...
	for i := range length {
		...
		result = append(result, struct {
			...
			Resources:            resources,
			AdjustmentWindowSecs: uint32(window),
			TargetPerSec:         target,
			Backlog:              backlog,
		})
	}
